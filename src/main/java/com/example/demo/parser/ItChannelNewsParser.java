...
@Component
@Slf4j
public class ItChannelNewsParser {
    ...
    private static final String DOMAIN = "https://www.novostiitkanala.ru";
    private static final String BLOG_LINK = DOMAIN + "/news";
    ...
    private List<String> extractArticleLinks(Document page) {
        ...
        for (Element a : page.select(".elements-list a")) {
    ...
    private Optional<ArticleDTO> parseArticle(String url) {
        try {
            ...
            String date = Optional.ofNullable(doc.selectFirst("p"))
                    .map(Element::text)
                    .orElse("Дата не указана");

            Element firstParagraph = doc.selectFirst("p");
            if (firstParagraph != null) {
                firstParagraph.remove(); // удаляем первый параграф
            }

            StringBuilder fullText = new StringBuilder();
            for (Element p : doc.select("p")) {
                String txt = p.text().trim();
                if (!txt.isEmpty()) fullText.append(txt).append(' ');
            }
